...
for NODE in "${NODES[@]}"; do
  ACTUAL=$(run_on_node "$NODE" "hostname")
  if [[ "$ACTUAL" != "$NODE" ]]; then
...
for i in "${!NODES[@]}"; do
  NODE="${NODES[$i]}"
  EXPECTED_IP="${IPS[$i]}"
  ACTUAL_IP=$(run_on_node "$NODE" "ip -4 addr | grep -oP '(?<=inet\s)\d+(\.\d+){3}' | grep 192.168.122")
...
for NODE in "${NODES[@]}"; do
  for TARGET in "${NODES[@]}"; do
    run_on_node "$NODE" "getent hosts $TARGET" >/dev/null || {
...
for NODE in "${NODES[@]}"; do
  for TARGET in "${NODES[@]}"; do
    run_on_node "$NODE" "ping -c 1 -W 1 $TARGET" >/dev/null || {
...
for NODE in "${NODES[@]}"; do
  run_on_node "$NODE" "sudo -n true" >/dev/null 2>&1 || {
...
for NODE in "${NODES[@]}"; do
  run_on_node "$NODE" "sudo dnf install -y openmpi openmpi-devel gcc gcc-c++"
done
